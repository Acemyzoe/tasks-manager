...
#define TRANSWARP_CPP11 1
    ...
    inline std::string to_string(const transwarp::task_type &type)
    {
        switch (type)
        {
        case transwarp::task_type::root:
            return "root";
        case transwarp::task_type::accept:
            return "accept";
        case transwarp::task_type::accept_any:
            return "accept_any";
        case transwarp::task_type::consume:
            return "consume";
        case transwarp::task_type::consume_any:
            return "consume_any";
        case transwarp::task_type::wait:
            return "wait";
        case transwarp::task_type::wait_any:
            return "wait_any";
        }
    ...
    inline std::string to_string(const transwarp::itask &task, transwarp::str_view separator = "\n")
    {
        std::string s;
        s += '"';
    ...
    inline std::string to_string(const transwarp::edge &edge, transwarp::str_view separator = "\n")
    {
    ...
    inline std::string to_string(const std::vector<transwarp::edge> &edges, transwarp::str_view separator = "\n")
    {
    ...
    namespace detail
    {
        ...
        void apply_to_each(Functor &&f, Tuple &&t)
        {
            ...
            std::apply([&f](auto &&...arg)
                       { (..., std::forward<Functor>(f)(std::forward<decltype(arg)>(arg))); },
                       std::forward<Tuple>(t));
#endif
        }
        ...
        struct call_impl
        {
            ...
            {
                return call_impl<TaskType, total == 1 + static_cast<int>(sizeof...(n)), total, n..., static_cast<int>(sizeof...(n))>::template work<Result>(task_id, task, parents);
            }
        };
        ...
        Result call(std::size_t task_id, const Task &task, const std::tuple<std::shared_ptr<transwarp::task<ParentResults>>...> &parents)
        {
            ...
            return transwarp::detail::call_impl<TaskType, 0 == n, static_cast<int>(n)>::template work<Result>(task_id, task, parents);
        }
        ...
        {
            return transwarp::detail::call_impl_vector<TaskType>::template work<Result>(task_id, task, parents);
        }
    ...
    auto for_each(InputIt first, InputIt last, UnaryOperation unary_op) -> std::shared_ptr<transwarp::task_impl<transwarp::wait_type, transwarp::no_op_functor, std::vector<std::shared_ptr<transwarp::task<void>>>>>
    {
        ...
        {
            tasks.push_back(transwarp::make_task(transwarp::root, [unary_op, first]
                                                 { unary_op(*first); }));
        }
    ...
    auto transform(InputIt first1, InputIt last1, OutputIt d_first, UnaryOperation unary_op) -> std::shared_ptr<transwarp::task_impl<transwarp::wait_type, transwarp::no_op_functor, std::vector<std::shared_ptr<transwarp::task<void>>>>>
    {
        ...
        {
            tasks.push_back(transwarp::make_task(transwarp::root, [unary_op, first1, d_first]
    ...
    public:
        ...
        explicit task_pool(std::shared_ptr<transwarp::task<ResultType>> task)
        ...
        void reclaim()
        {
            ...
            {
                const transwarp::itask *task = finished.front();
                finished.pop();
    ...
    private:
        ...
        {
        public:
            explicit finished_listener(task_pool<ResultType> &pool)
                : pool_(pool)
            {
            }
