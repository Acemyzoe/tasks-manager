...
namespace
{
    ...
    {
        auto gen_task = tw::make_task(tw::root, [gen]() -> std::mt19937 &
                                      { return *gen; });
...
namespace examples
{
    ...
    void benchmark_statistical(std::ostream &os, std::size_t sample_size)
    {
        ...
        std::ofstream("benchmark_statistical.dot") << tw::to_string(task->edges());

        const auto func_us = measure([gen]
                                     { calculate_via_functions(*gen); },
                                     sample_size);

        const auto tw_us = measure([task]
                                   { calculate_via_transwarp(*task); },
                                   sample_size);

        os << "functions: " << func_us << " us" << std::endl;
        os << "transwarp: " << tw_us << " us" << std::endl;
        os << "difference: " << static_cast<double>(tw_us - func_us) / static_cast<double>(func_us) * 100. << " %" << std::endl;
    }
